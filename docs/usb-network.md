...
COM4 later returned with PnP status `OK`, but one passive 115200 8N1 open with DTR/RTS off failed with Windows' `A device attached to the system is not functioning.` No bytes were sent or received; the serial baud rate and functionality remain unknown. Windows' `MaxBaudRate: 115200` is a driver capability field, not an observed device speed. No further rates were attempted after the open failure.
