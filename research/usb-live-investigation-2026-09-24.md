...
**Later same-day resolution:** A Microsoft WHCP-signed Acer RNDIS package from [Microsoft Update Catalog](https://www.catalog.update.microsoft.com/ScopedViewInline.aspx?updateid=38c84d13-eb1b-4327-aae5-e36e8cb7933d) exactly matched `USB\VID_0525&PID_A4A2`. After its installation and one cable reconnect, the camera became a Net-class `USB Ethernet/RNDIS Gadget` and Windows created a working adapter. A temporary host address `192.168.2.1/24` on that adapter enabled ping and OSC HTTP at `192.168.2.2`; one short stream produced successful RTSP `OPTIONS`/`DESCRIBE` over USB. No media frames were decoded. The detailed package, IP, results, and recovery notes are in [USB networking](../docs/usb-network.md). The material below documents the **earlier pre-fix comparison**, not the current final state.

**Pre-fix controlled cable-#3 before/unplug/reconnect comparison:** The Bublcam enumerated once as `USB\VID_0525&PID_A4A2`, class `Ports`, friendly name `USB Serial Device (COM4)`. Its bus-reported description was `RNDIS/Ethernet Gadget`, but Windows bound `usbser`/`usbser.inf` and created **no USB network adapter at that stage**. After unplugging, COM4 disappeared. On reconnection at the same hub port, Windows instead reported a device-descriptor failure (Code 43); COM4 did not return. No `192.168.2.x` host interface or route was observed in those snapshots. USB Ethernet, OSC, and RTSP were unverified **at the time of this comparison**; the later successful tests are summarized above. Do not treat the failure pseudo-ID `VID_0000&PID_0002` as the camera's real VID/PID.

During the controlled comparison described below, no USB driver, camera setting, Windows network setting, firmware, or gadget configuration was changed. No camera reboot, USB stream, photo, recording, credential attempt, COM-port write, or broad port scan occurred in that comparison. Later signed driver, host IP, OSC, and short RTSP tests are documented above and in the USB networking guide. Raw camera media and device serials are not included here.
...
At this earlier stage, USB could not be recommended as the main development/control connection on this Windows setup. The descriptor failure stopped probing until the camera enumerated again. The later exact-ID signed driver and successful network/API tests supersede that earlier limitation. On any other host, establish adapter identity and route before pinging `192.168.2.2` or probing OSC/RTSP; do not install an unrelated driver or alter gadget mode as a discovery step.
